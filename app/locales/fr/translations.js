export default {
    "about": {
        "title": "A propos"
    },
    "access": "Accès",
    "access-overview": {
        "title": "Récapitulatif des accès"
    },
    "access-point": {
        "create": {
            "title": "Creér un point d'accès"
        },
        "list": {
            "title": "Liste des point d'accès"
        },
        "title": "Point d'accès"
    },
    "access_point": "Point d'accès",
    "access_point_placeholder": "MyAccessPoint",
    "access_points": "Points d'accès",
    "accesspoint": {
        "belongs_to": "Ce point d'accès appartient à la porte <strong>{{dooralias}}</strong>.",
        "belongs_to_nolink": "Ce point d'accès appartient à la porte",
        "create": "Ajouter un point d'accès",
        "delete_confirm": "Êtes-vous sur de vouloir supprimer ce point d'accès ?",
        "no_belongs": "Ce point d'accès n'est pas lié à une porte."
    },
    "acknowledge": "Acquitter",
    "acknowledged": "Acquittée",
    "action": "Action",
    "add": "Ajouter",
    "add_time_zone": "Ajouter plage horaire",
    "administrator": "Administrateur",
    "alias": "Alias",
    "alias_": "Alias:",
    "alias_placeholder": "Un nom mémorisable",
    "audit": {
        "after": "Après :",
        "before": "Avant :",
        "details_intro": "un état avant / après de l'enregistrement d'audit de l'objet modifié. Merci de noter que cette fonctionnalité n'est pas disponibles pour tous les types d'enregistrement d'audit.",
        "enable": "Activer :",
        "enable_credentialevent": "CredentialEvent",
        "enable_doorevent": "DoorEvent",
        "enable_groupevent": "GroupEvent",
        "enable_scheduleevent": "ScheduleEvent",
        "enable_updateevent": "UpdateEvent",
        "enable_userevent": "UserEvent",
        "enable_usergroupevent": "UserGroupMembershipEvent",
        "enable_wsapicall": "WSAPICall",
        "enable_zoneevent": "ZoneEvent",
        "entries_below": "Enregistrements d'audit ci-dessous :",
        "title": "Log d'audit"
    },
    "audit_log": "Log d'audit",
    "author": "Auteur",
    "autocomplete": {
        "search_access_point": "Veuillez choisir un point d'accès (tapez % pour tout voir)",
        "search_group": "Veuillez choisir un groupe (tapez % pour tout voir)"
    },
    "buzzer": "Buzzer",
    "cancel": "Annuler",
    "cancel_and_refresh": "Annuler et rafraîchir",
    "cancelled": "Annulé",
    "clear_selection": "Effacer la selection",
    "config_version": "Configuration Version",
    "config_version_descr": "Est utilisé lors de la synchronisation afin de déterminer ce qui nécessite synchronisation.",
    "configuration": "Configuration",
    "configurations": {
        "delete_confirm": "Êtes-vous sûr de vouloir supprimer cette configuration ?",
        "error": {
            "create_error": "Une erreur est survenue lors de la création de la configuration.",
            "create_success": "Configuration correctement crée.",
            "remove_error": "Une erreur est survenue lors de la suppression de la configuration.",
            "remove_success": "Configuration correctement supprimée.",
            "update_error": "Une erreur est survenue lors de la mise à jour de la configuration.",
            "update_success": "Configuration correctement mise à jour."
        }
    },
    "confirm": "Confirmer",
    "confirmation": "Confirmation",
    "connected": "Connecté",
    "controller_module": "Module contrôleur",
    "count": "Nombre",
    "credential": {
        "noowner": "Le crédential n'a pas de propriétaire.",
        "title": "Informations de crédential"
    },
    "credentials": {
        "delete_confirm": "Êtes-vous sûr de vouloir supprimer ce crédential ?",
        "list": {
            "title": "Liste des crédentiaux"
        },
        "pin_code": {
            "add": "Ajouter un code PIN",
            "title": "Code PIN"
        },
        "pin_code_create": {
            "title": "Créer Code PIN"
        },
        "rfid_card": {
            "add": "Ajouter une carte RFID",
            "error": "Format invalide, il devrait y avoir une suite de deux nombres hexadécimaux(0-9A-F), séparé par deux-points(:). Exemple : '0A:2b:8E:9f'",
            "hex_id": "Identifiant de carte hexadécimal",
            "number_bits": "Nombre de bits",
            "title": "Carte RFID"
        },
        "rfid_card_create": {
            "title": "Créer Carte RFID"
        },
        "title": "Crédentiaux"
    },
    "current_page": "Page actuelle",
    "date": "Date",
    "default_value": "Valeur par défaut",
    "delete": "Supprimer",
    "delete_group_confirmation": "Etes vous sur de vouloir supprimer ce groupe ?",
    "description": "Description",
    "deselect_all": "Tout désélectionner",
    "details": "Détails",
    "device": "Device",
    "direction": "Direction",
    "disconnected": "Déconnecté",
    "display_id": "Identifiant affiché",
    "door": {
        "create": {
            "title": "Ajouter une porte"
        },
        "create_door": "Ajouter une porte",
        "delete_confirm": "Êtes-vous sûr de vouloir supprimer cette porte ?",
        "list": {
            "title": "Liste des portes"
        },
        "placeholder": "MySuperDoor",
        "title": "Porte"
    },
    "doors": "Portes",
    "edit": "Editer",
    "email": "Email",
    "enable": "Activer",
    "enabled": "Actif",
    "end_date": "Date de fin",
    "error": {
        "entity_id": "Identifiant d'entité :",
        "entity_not_found": "Entité non trouvée.",
        "entity_not_found_descr": "L'entité demandée n'a pas pu être trouvée par le serveur.",
        "entity_type": "Type d'entité :",
        "message": "Message :",
        "permission_denied": "Permission refusée.",
        "permission_denied_descr": "La requète effectuée a été refusée pour des raisons de permission. <br>Si vous pensez que ceci est incorrecte, merci de contacter votre administrateur et de lui fournir cet identifiant de requète : {{reqid}}<br>Si vous êtes l'administrateur, vous devriez regarder les logs pour plus de détails.",
        "request_timeout": "Temps d'attente dépassé.",
        "request_timeout_descr": "La requète n'a pas recu de réponse dans un temps raisonable et a expiré. Cette erreur <i>peut</i> indiquer des problèmes avec votre connexion internet.",
        "requestid": "Identifiant de requête :",
        "session_aborted": "Session annulée.",
        "session_aborted_descr": "Vous devriez être redirigé vers la page d'authentification dans 5 secondes. Si ceci n'est pas le cas, merci de cliquer",
        "statuscode": "Status Code :",
        "title": "Oh non, quelque chose s'est mal passé.",
        "unknown_error": "Erreur inconnue.",
        "unknown_error_descr": "Désolé, nous ne savons pas ce qu'il s'est passé."
    },
    "errors": {
        "accepted": "{{description}} doit être valide",
        "after": "{{description}} doit être après {{after}}",
        "before": "{{description}} doit être avant {{before}}",
        "blank": "{{description}} ne devrait pas être vide",
        "collection": "{{description}} doit être une collection",
        "confirmation": "{{description}} ne correspond pas à {{on}}",
        "date": "{{description}} doit être une date valide",
        "description": "Ce champ",
        "email": "{{description}} doit être une adresse email valide",
        "empty": "{{description}} ne devrait pas être vide",
        "equalTo": "{{description}} doit être égale a {{is}}",
        "even": "{{description}} doit être pair",
        "exclusion": "{{description}} est réservé",
        "greaterThan": "{{description}} doit être supérieur à {{gt}}",
        "greaterThanOrEqualTo": "{{description}} doit être supérieur ou égale à {{gte}}",
        "inclusion": "{{description}} n'est pas inclus dans la liste",
        "invalid": "{{description}} est invalide",
        "lessThan": "{{description}} doit être inférieur à {{lt}}",
        "lessThanOrEqualTo": "{{description}} doit être inférieur ou égale a {{lte}}",
        "notANumber": "{{description}} doit être un nombre",
        "notAnInteger": "{{description}} doit être un entier",
        "odd": "{{description}} doit être impair",
        "onOrAfter": "{{description}} doit être maintenant ou après {{onOrAfter}}",
        "onOrBefore": "{{description} doit être maintenant ou avant {{onOrBefore}}",
        "otherThan": "{{description}} doit être différents de {{value}}",
        "phone": "{{description}} doit être un numéro de telephone valide",
        "positive": "{{description}} doit être positif",
        "present": "{{description}} doit être vide",
        "singular": "{{description}} ne peut pas être une collection",
        "tooLong": "{{description}} est trop long (maximun de {{max}} caractères)",
        "tooShort": "{{description}} est trop court (minimum de {{min}} caractères)",
        "url": "{{description}} doit être une URL valide",
        "wrongDateFormat": "{{description}} doit être au format {{format}}",
        "wrongLength": "{{description}} est de mauvaise longueur (doit être de {{is}} caractères)"
    },
    "evoxs": {
        "access-point": {
            "title": "Point d'accès EvoXS"
        },
        "cylinder-events": {
            "title": "Evenements cylindre"
        },
        "lockid": "Lock Id",
        "operations": {
            "extract_event": "Récupérer les évenements",
            "set_time": "Configurer l'heure du cylindre",
            "title": "Opérations"
        },
        "title": "EvoXS",
        "update": {
            "title": "Mise à jour d'un point d'accès EvoXS"
        }
    },
    "finalized": "Finalisé",
    "firstname": "Prénom",
    "form": {
        "password": {
            "label": "Mot de passe",
            "ph": "Votre mot de passe"
        },
        "username": {
            "label": "Nom d'utilisateur",
            "ph": "Thomas"
        }
    },
    "general_information": "Informations sur Leosac",
    "gpio": "GPIO",
    "group": {
        "create": {
            "title": "Créer un groupe"
        },
        "list": {
            "title": "Liste des groupes"
        },
        "title": "Groupe"
    },
    "group_information": "Information de groupe",
    "groups": "Groupes",
    "hardware": "Matériels",
    "hardware_update": "Mises à jour",
    "here": "ici",
    "index": {
        "apply_update": "Appliquer les MàJ",
        "event": "Evènements",
        "manage_access": "Gérer les accès",
        "title": "Index",
        "welcome": "Bienvenue <strong>{{username}}</strong> <br> Gerez vos cylindres EvoXS et les permissions utilisateurs. <br> Installez, configurez et deployez depuis l'interface web."
    },
    "information": "Information",
    "informations": {
        "card_id": "Vu que nous devons fournir une suite de nombre hexadécimals suivis de double point, cela doit suivre le format suivant : 00:b1:cC:9f ou l'expression régulière: /[0-9A-F]{2}(?::[0-9A-F]{2})*$/i .",
        "enable": "Activer cet appareil",
        "number_of_bit": "Le nombre de bits définit la longueur de l'identifiant hexadécimal ci-dessus. Pour votre information, un nombre hexadécimal est codé sur 4 bits. Les doubles points n'entrent pas dans ce compte, mais doivent quand même être écrits."
    },
    "instance_name": "Nom de l'instance",
    "instance_name_descr": "Le nom unique de l'instance {{appname}}.",
    "kick": "Expulser",
    "kick_group_confirmation": "Etes vous sur de vouloir expulser cet utilisateur du groupe ?",
    "language": "Langue",
    "lastname": "Nom de famille",
    "leave": "Quitter",
    "leave_group_confirmation": "Etes vous sur de vouloir quitter ce groupe ?",
    "led": "LED",
    "legend": {
        "logical": "Logiques",
        "logicalRoot": "Parent des zones logiques",
        "physical": "Physiques",
        "physicalRoot": "Parent des zones physiques",
        "root": "Racine par défaut",
        "title": "Légende"
    },
    "leosac_builtin_ap": {
        "action": {
            "error_actions": "Actions erreurs",
            "success_actions": "Action réussite",
            "targeted_device": "Appareil visé",
            "command": "Commande de l'action",
            "command_placeholder": "Choisissez la commande",
            "add_success": "Ajoutés aux actions réussites",
            "add_error": "Ajoutés aux actions échoués",
            "action_number": "Action {{command}} sur {{device}}",
            "title": "Interface de gestion des actions",
            "command_picker": "Sélectionner une commande",
            "create_action": "Créer une action"
        },
        "add_close_schedule": "Ajouter aux horaires fermés",
        "add_device": "Ajouter un appareil",
        "add_open_schedule": "Ajouter aux horaires ouvertes",
        "always_close_schedules": "Horaires toujours fermés",
        "always_open_schedules": "Horaires toujours ouvertes",
        "authentication_sources_device": "Appareils d'authentification",
        "device_picker": "Sélectionner un appareil",
        "device_placeholder": "Taper le nom de l'appareil(% pour tout)",
        "schedule_picker": "Sélectionner une horaire",
        "schedules": "Horaires",
        "schedules_placeholder": "Choisissez une horaire",
        "title": "Point d'accès intégré à Leosac"
    },
    "link_to_profil": "Lien vers le profil d'utilisateur",
    "loading": {
        "message": "La page est en cours de chargement. Cela ne devrait prendre qu'un instant. <br> Si ce n'est pas le cas, rechargez la page.",
        "title": "Veuillez patienter"
    },
    "logicalRoot": "Zones logiques",
    "login": "Se connecter",
    "login-form": {
        "submit": "Connexion",
        "username_password_required": "Le nom d'utilisateur et le mot de passe sont requis.",
        "welcome": "Veuillez vous connecter"
    },
    "logout": "Se déconnecter",
    "manager": "Manageur",
    "mapping": "Mapping",
    "mask": "Masque",
    "member": "Membre",
    "members": "Membres",
    "message": "Message",
    "minutes_ago": "il y a {{minutes}} minutes",
    "module": "Module",
    "modules": "Modules",
    "modules_both": "Modules chargés",
    "modules_both_desc": "Modules fonctionnant à travers le serveur et le client.",
    "modules_client": "Modules du client",
    "modules_client_desc": "Quels modules sont chargés du côté du client.",
    "modules_client_error": "Ces modules devraient également être chargés du serveur pour fonctionner correctement: ",
    "modules_desc": "Les modules qui sont lancés.",
    "modules_server": "Modules du serveur",
    "modules_server_desc": "Quels modules sont chargés du côté du serveur.",
    "modules_server_error": "Ces modules devraient également être chargés du client pour fonctionner correctement: ",
    "modules_wizard": "Assistants d'installation",
    "modules_wizard_desc": "Quels sont les modules dont à besoin les assistants d'installation.",
    "name": "Nom",
    "name_app": "Nom de l'application",
    "no": "Non",
    "not_available": "Indisponible",
    "notes": "Notes",
    "notes_placeholder": "Quelques notes optionnelles...",
    "on": "sur",
    "operator": "Opérateur",
    "other": "Autre",
    "overview": {
        "intro": "Cette page fournie une vue globale de certaines valeurs de configuration du système {{appname}}.",
        "last_logs": "Derniers évènements",
        "logs_per_page": "Evènements par page",
        "total_logs": "Il y a un total de {{totalLogs}} évènements."
    },
    "owner": "Propriétaire",
    "owner_": "Propriétaire :",
    "page_size": "Taille de la page",
    "parameter": "Paramètre",
    "parameters": "Paramètres",
    "password": "Mot de passe",
    "password-change": {
        "current_pw": "Mot de passe actuel",
        "new_pw": "Nouveau mot de passe",
        "submit": "Changer le mot de passe",
        "successfully_changed": "Mot de passe changé avec succès."
    },
    "pending": "En attente",
    "pending_updates": "Mises a jour en attente",
    "physicalRoot": "Zones physiques",
    "pincode": "Code Pin",
    "profile": {
        "fail_update": "Erreur de mise à jour du profile",
        "profile_updated": "Le profile a été mis à jour",
        "title": "Profil Utilisateur"
    },
    "rank": "Rang",
    "rank_group": "Rang dans le groupe",
    "reader": "Lecteur",
    "refresh": "Actualiser",
    "remove": "Enlever",
    "remove_selected": "Supprimer la sélection",
    "remove_timeframe": "Supprimer la zone de temps",
    "restart": "Redémarrage",
    "restart_info": "Ce bouton permet de redémarrer le serveur",
    "save": "Enregistrer",
    "schedule": {
        "add_door": "Ajouter une porte",
        "add_mapping": "Ajouter mapping",
        "asfield": "Horaires :",
        "create": {
            "title": "Créer une horaire"
        },
        "create_note": "Merci de compléter le formulaire suivant afin de créer une nouvelle horaire.",
        "create_schedule": "Créer une horaire",
        "credential_schedule": "Affiche les horaires et leur mapping qui sont liés <i>directement</i> au crédential.",
        "delete_confirm": "Êtes-vous sûr de vouloir supprimer cette horaire ?",
        "doors": "Portes",
        "end_time": "Heure de fin",
        "friday": "Vendredi",
        "grant_to_doors": "Autorise les accès aux portes suivantes :",
        "group_schedule": "Affiche les horaires et leur mapping qui sont liés <i>directement</i> au groupe.",
        "join_mapping": "Rejoindre le mapping",
        "list": {
            "title": "Liste des horaires"
        },
        "mapping": "Mapping : {{alias}}",
        "mapping_list": "Ci-dessous la liste des mapping liés à l'horaire.",
        "monday": "Lundi",
        "no_doors": "Ce mapping n'est lié à aucune porte. Cela veut dire qu'il n'aura aucun effet.",
        "no_mapping": "Il n'y a pas de mapping",
        "no_timeframe": "Ce programme n'a pas de zones horaires. Ceci veut dire qu'il n'aura aucun effet.",
        "placeholder": "MonHoraire",
        "remove_from_mapping": "Supprimer du mapping",
        "remove_from_mapping_confirm": "Êtes-vous sûr de vouloir quitter ce mapping ?",
        "saturday": "Samedi",
        "start_time": "Heure de début",
        "sunday": "Dimanche",
        "thursday": "Jeudi",
        "title": "Horaire",
        "tuesday": "Mardi",
        "type": "Saisissez le nom d'une horraire (ou %)",
        "type_door": "Commencez à saisir l'alias de la porte",
        "user_schedule": "Affiche les horaires et leur mapping qui vous sont liés <i>directement</i>. Ceci n'affichage pas les horaires qui sont liés au travers les groupes ou les crédentiaux.",
        "wednesday": "Mercredi"
    },
    "schedules": "Horaires",
    "search": "Rechercher",
    "security": "Securité",
    "select_all": "Tout sélectionner",
    "settings": "Options",
    "start_date": "Date de début",
    "submit": "Soumettre",
    "summary": "Récapitulatif",
    "supervisor": "Superviseur",
    "switch": "Changer de langue",
    "switch_en": "Switch to english",
    "switch_fr": "Passer en francais",
    "system-overview": {
        "title": "Vue générale"
    },
    "table": {
        "group_id": "#Id du groupe",
        "group_name": "Nom du groupe",
        "join_date": "Date d'enregistrement",
        "total_member": "Nombre de membres"
    },
    "test": "test",
    "timeframe": "Zone de temps",
    "timeframes": "Zones de temps",
    "timeframes_": "Zones de temps",
    "timestamp": "Timestamp",
    "to": "à",
    "toggle_all": "Tout basculer",
    "total_page": "Total des pages",
    "tree-view": {
        "title": "Récapitulatif des zones"
    },
    "type": "Type",
    "typeDoor": "Porte",
    "typeGroup": "Groupe",
    "typePinCode": "Code pin",
    "typeRfidCard": "Carte RFID",
    "typeSchedule": "Planning",
    "typeUser": "Utilisateur",
    "typeZone": "Zone",
    "type_zone": "Type de zone (peut-être logique ou physique)",
    "unknown": "Inconnu",
    "update": {
        "check_update": "Rechercher les mises à jour",
        "everything_up_to_date": "Tout est à jour.",
        "filter_access_point": "Filtrer par point d'accès",
        "pending_check_update": "Recherche des mises à jour en cours.",
        "potential_update": "Potentielle mises à jour",
        "status": "état",
        "table": {
            "checkpoint": "Checkpoint",
            "generated_at": "Date de génération",
            "target_alias": "Alias du point d'accès",
            "update_needed": "Mise à jour requise ?",
            "updated_at": "Date de changement de status"
        },
        "title": "Gestion des mises à jour",
        "update_acked": "La mise à jour a été acquittée.",
        "update_acked_failed": "L'acquitement de la mise à jour a echoué.",
        "update_cancel_failed": "L'annulation de la mise à jour a echoué.",
        "update_cancelled": "La mise à jour a été annulée."
    },
    "update_history": "Historique des mises à jour",
    "uptime": "Uptime",
    "uptime_descr": "Temps depuis que {{appname}} a démarré. (HH:MM:SS)",
    "url": "Url",
    "user": "Utilisateur",
    "username": "Nom d'utilisateur",
    "username_": "Nom d'utilisateur :",
    "users": {
        "add_credential": "Ajouter un crédential",
        "add_group": "Ajouter un groupe",
        "add_to_group": "Ajouter au groupe",
        "add_user": "Ajouter un utilisateur",
        "create": {
            "account_info": "Information du compte",
            "fail_create": "Echec de la creation du nouvel utilisateur.",
            "title": "Ajouter un utilisateur",
            "user_create": "User crée avec succès."
        },
        "create_group": "Créer le groupe",
        "create_new_group": "Créer un nouveau groupe",
        "create_new_user": "Ajouter un utilisateur",
        "delete_group_confirm": "Êtes-vous sûr de voulour supprimer ce groupe ?",
        "error": {
            "add_error": "Erreur durant l'ajout de l'utilisateur au groupe",
            "add_success": "Utilisateur correctement ajouté au groupe",
            "find_error": "N'arrive pas à trouver cet utilisateur."
        },
        "group_info": "Information sur le groupe",
        "group_name": "Nom du groupe",
        "list": {
            "title": "Liste des utilisateurs"
        },
        "list_groups_notice": "Merci de noter que le \"nombre total de membres\" peut ne pas refléter le nombre de membres réel. Ceci est lié aux permissions.",
        "members": "Nombre total de membres",
        "remove_from_group": "Êtes-vous sûr de vouloir supprimer {{username}} du groupe ?",
        "title": "Utilisateurs",
        "type_group": "Commencez à saisir le nom d'un groupe",
        "type_user": "Commencez à saisir le nom d'un utilisateur"
    },
    "validity": "Validité",
    "value": "Valeur",
    "viewer": "Spectateur",
    "w": {
        "card_csn": "Numéro de la carte",
        "credentials": "Crédentiaux",
        "door": "Porte",
        "overview": "Vue générale",
        "room_number": "Numéro de la pièce",
        "severity": "Sévérité",
        "state": "Etat",
        "update": "Mettre à jour",
        "user_id": "Id utilisateur"
    },
    "wait": "Merci de patienter, récupération et préparation des données.",
    "yes": "Oui",
    "zone": {
        "add_children": "Ajouter une zone enfant",
        "child": "Enfants",
        "create": {
            "title": "Ajouter une zone"
        },
        "create_zone": "Ajouter une zone",
        "delete_confirm": "Êtes-vous sûr de vouloir supprimer cette zone ?",
        "error": {
            "edited_error": "Une erreur est survenue durant l'édition de la zone",
            "edited_success": "La zone a été éditée avec succès",
            "error_added": "Une erreur est survenue durant l'ajout de la porte",
            "error_removed": "Une erreur est survenue durant la suppression de la porte",
            "successfully_added": "Porte ajouté avec succès",
            "successfully_removed": "Porte supprimé avec succès"
        },
        "list": {
            "title": "Liste des zones"
        },
        "placeholder": "MySuperZone",
        "remove_from_zone": "Supprimer de la zone ?",
        "title": "Zone",
        "tree_view": "Récapitulatif des zones",
        "type": {
            "format": "0: type logique, 1: type physique",
            "logical": "Logique",
            "physical": "Physique"
        },
        "type_zone": "Commencez à saisir l'alias de la zone enfant"
    },
    "zones": "Zones"
};
