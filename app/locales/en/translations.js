export default {
    "about": {
        "title": "About"
    },
    "access": "Access",
    "access-overview": {
        "title": "Access Overview"
    },
    "access-point": {
        "create": {
            "title": "Create Access Point"
        },
        "list": {
            "title": "Access Point list"
        },
        "title": "Access Point"
    },
    "access_point": "Access Point",
    "access_point_placeholder": "MyAccessPoint",
    "access_points": "Access Points",
    "accesspoint": {
        "belongs_to": "This access point belongs to door <strong>{{dooralias}}</strong>.",
        "belongs_to_nolink": "This access point belongs to a door",
        "create": "Add Access Point",
        "delete_confirm": "Are you sure you wish to delete this access point?",
        "no_belongs": "This access point is not yet tied to a door."
    },
    "acknowledge": "Acknowledge",
    "acknowledged": "Acknowledged",
    "action": "Action",
    "add": "Add",
    "add_time_zone": "Add Time Frame",
    "administrator": "Administrator",
    "alias": "Alias",
    "alias_": "Alias:",
    "alias_placeholder": "A friendly name to remember me by",
    "audit": {
        "after": "After:",
        "before": "Before:",
        "details_intro": "This show a before and after the audit entry snapshot of the object that was modified. Note that this features is not available for all kind of audit entry.",
        "enable": "Enable :",
        "enable_credentialevent": "CredentialEvent",
        "enable_doorevent": "DoorEvent",
        "enable_groupevent": "GroupEvent",
        "enable_scheduleevent": " ScheduleEvent",
        "enable_updateevent": "UpdateEvent",
        "enable_userevent": "UserEvent",
        "enable_usergroupevent": "UserGroupMembershipEvent",
        "enable_wsapicall": "WSAPICall",
        "enable_zoneevent": "ZoneEvent",
        "entries_below": "Audits entries below:",
        "title": "Audit Trail"
    },
    "audit_log": "Audit Log",
    "author": "Author",
    "autocomplete": {
        "search_access_point": "Pick an access point (type % to list all)",
        "search_group": "Pick a group (type % to list all)"
    },
    "buzzer": "Buzzer",
    "cancel": "Cancel",
    "cancel_and_refresh": "Cancel & Refresh",
    "cancelled": "Cancelled",
    "clear_selection": "Clear selection",
    "config_version": "Configuration Version",
    "config_version_descr": "Is used when performing synchronization to determine whether or not synchronization is needed.",
    "configuration": "Configuration",
    "configurations": {
        "delete_confirm": "Are you sure that you want to delete this config",
        "error": {
            "create_error": "An error occurred while creating the config.",
            "create_success": "Config correctly created.",
            "remove_error": "An error occurred while removing the config.",
            "remove_success": "Config correctly removed.",
            "update_error": "An error occurred while updating the config.",
            "update_success": "Config correctly updated."
        }
    },
    "confirm": "Confirm",
    "confirmation": "Confirmation",
    "connected": "Connected",
    "controller_module": "Controller Module",
    "count": "Count",
    "credential": {
        "noowner": "The credential has no owner.",
        "title": "Credential information"
    },
    "credentials": {
        "delete_confirm": "Are you sure you wish to delete this credential?",
        "list": {
            "title": "Credential list"
        },
        "pin_code": {
            "add": "Add PinCode",
            "title": "PIN Code"
        },
        "pin_code_create": {
            "title": "Create PIN Code"
        },
        "rfid_card": {
            "add": "Add RFIDCard",
            "error": "Invalid format, should be a sequence of two hexadecimal number(0-9A-F), separated by colon(:). Exampe: '0A:2b:8E:9f'",
            "hex_id": "Hexadecimal Card Identifier",
            "number_bits": "Number of bits",
            "title": "RFID Card"
        },
        "rfid_card_create": {
            "title": "Create RFID Card"
        },
        "title": "Credentials"
    },
    "current_page": "Current Page",
    "date": "Date",
    "default_value": "Default value",
    "delete": "Delete",
    "delete_group_confirmation": "Are you sure you wish to delete this group ?",
    "description": "Description",
    "deselect_all": "Deselect all",
    "details": "Details",
    "device": "Device",
    "direction": "Direction",
    "disconnected": "Disconnected",
    "display_id": "Display Identifier",
    "door": {
        "create": {
            "title": "Create door"
        },
        "create_door": "Add Door",
        "delete_confirm": "Are you sure you wish to delete this door ?",
        "list": {
            "title": "Door list"
        },
        "placeholder": "MySuperDoor",
        "title": "Door"
    },
    "doors": "Doors",
    "edit": "Edit",
    "email": "Email",
    "enable": "Enable",
    "enabled": "Enabled",
    "end_date": "End date",
    "error": {
        "entity_id": "Entity identifier:",
        "entity_not_found": "Entity not found.",
        "entity_not_found_descr": "The requested entity couldn't be found by the server.",
        "entity_type": "Entity type:",
        "message": "Message:",
        "permission_denied": "Permission denied.",
        "permission_denied_descr": "The request you made has been denied for permission reason. <br>If you believe this is incorrect, please contact your administrator and provide them with this request identifier: {{reqid}}<br>If you are the administrator, you'll have to resort to the log for possible details.",
        "request_timeout": "Request timeout.",
        "request_timeout_descr": "The request didn't received a reply in a reasonable time, and has now timed out. This error <i>may</i> indicates problems with your internet connection.",
        "requestid": "Request Identifier:",
        "session_aborted": "Session aborted.",
        "session_aborted_descr": "You should be redirected to the login page in 5 seconds. If that's not the case, please click",
        "statuscode": "Status Code:",
        "title": "Oh no, something went wrong.",
        "unknown_error": "Unknown error.",
        "unknown_error_descr": "Sorry, we don't know what happened."
    },
    "errors": {
        "accepted": "{{description}} must be accepted",
        "after": "{{description}} must be after {{after}}",
        "before": "{{description}} must be before {{before}}",
        "blank": "{{description}} can't be blank",
        "collection": "{{description}} must be a collection",
        "confirmation": "{{description}} doesn't match {{on}}",
        "date": "{{description}} must be a valid date",
        "description": "This field",
        "email": "{{description}} must be a valid email address",
        "empty": "{{description}} can't be empty",
        "equalTo": "{{description}} must be equal to {{is}}",
        "even": "{{description}} must be even",
        "exclusion": "{{description}} is reserved",
        "greaterThan": "{{description}} must be greater than {{gt}}",
        "greaterThanOrEqualTo": "{{description}} must be greater than or equal to {{gte}}",
        "inclusion": "{{description}} is not included in the list",
        "invalid": "{{description}} is invalid",
        "lessThan": "{{description}} must be less than {{lt}}",
        "lessThanOrEqualTo": "{{description}} must be less than or equal to {{lte}}",
        "notANumber": "{{description}} must be a number",
        "notAnInteger": "{{description}} must be an integer",
        "odd": "{{description}} must be odd",
        "onOrAfter": "{{description}} must be on or after {{onOrAfter}}",
        "onOrBefore": "{{description} must be on or before {{onOrBefore}}",
        "otherThan": "{{description}} must be other than {{value}}",
        "phone": "{{description}} must be a valid phone number",
        "positive": "{{description}} must be positive",
        "present": "{{description}} must be blank",
        "singular": "{{description}} can't be a collection",
        "tooLong": "{{description}} is too long (maximum is {{max}} characters)",
        "tooShort": "{{description}} is too short (minimum is {{min}} characters)",
        "url": "{{description}} must be a valid url",
        "wrongDateFormat": "{{description}} must be in the format of {{format}}",
        "wrongLength": "{{description}} is the wrong length (should be {{is}} characters)"
    },
    "evoxs": {
        "access-point": {
            "title": "EvoXS Access Point"
        },
        "cylinder-events": {
            "title": "Cylinder Events"
        },
        "lockid": "Lock Id",
        "operations": {
            "extract_event": "Extract events",
            "set_time": "Configure cylinder's time",
            "title": "Operations"
        },
        "title": "EvoXS",
        "update": {
            "title": "EvoXS Access Point Update"
        }
    },
    "finalized": "Finalized",
    "firstname": "First name",
    "form": {
        "password": {
            "label": "Password",
            "ph": "Your password"
        },
        "username": {
            "label": "Username",
            "ph": "John"
        }
    },
    "general_information": "Leosac information",
    "gpio": "GPIO",
    "group": {
        "create": {
            "title": "Create group"
        },
        "list": {
            "title": "Group list"
        },
        "title": "Group"
    },
    "group_information": "Group information",
    "groups": "Groups",
    "hardware": "Hardware",
    "hardware_update": "Hardware Update",
    "here": "here",
    "index": {
        "apply_update": "Apply Update",
        "event": "Events",
        "manage_access": "Manage Access",
        "title": "Dashboard",
        "welcome": "Welcome <strong>{{username}}</strong>.<br />Manage your EvoXS cylinders and user permissions.<br />Register, setup, deploy. All from a web-based interface.<br />"
    },
    "information": "Information",
    "informations": {
        "card_id": "Since this is a sequence of hexadecimal(0 to f) number, it should respect the following patern: 00:b1:cC:9f. It follow this regular expression: /[0-9A-F]{2}(?::[0-9A-F]{2})*$/i.",
        "enable": "Enable this device",
        "number_of_bit": "The number of bit define the length of the Hexadecimal Identifier above. For you information, an hexadecimal numeral is coded on 4 bits. The colons doesn't enter in our calculus, but they still need to be written."
    },
    "instance_name": "Instance Name",
    "instance_name_descr": "The (unique) name of the {{appname}} instance.",
    "kick": "Kick",
    "kick_group_confirmation": "Are you sure you wish to kick this user from the group ?",
    "language": "Language",
    "lastname": "Last name",
    "leave": "Leave",
    "leave_group_confirmation": "Are you sure you wish to leave this group ?",
    "led": "LED",
    "legend": {
        "logical": "Logical",
        "logicalRoot": "Parent of the logical zone",
        "physical": "Physical",
        "physicalRoot": "Parent of the physical zone",
        "root": "Default root",
        "title": "Legend"
    },
    "leosac_builtin_ap": {
        "action": {
            "error_actions": "Error Actions",
            "success_actions": "Success Actions",
            "targeted_device": "Targeted Device",
            "command": "Action Command",
            "command_placeholder": "Choose the command",
            "add_success": "Add to the success actions",
            "add_error": "Add to the error actions",
            "action_number": "Action {{command}} on {{device}}",
            "title": "Action management",
            "command_picker": "Command Picker",
            "create_action": "Create Action"
        },
        "add_close_schedule": "Add Close Schedules",
        "add_device": "Add Device",
        "add_open_schedule": "Add Open Schedules",
        "always_close_schedules": "Always Close Schedules",
        "always_open_schedules": "Always Open Schedules",
        "authentication_sources_device": "Authentication Source Devices",
        "device_picker": "Device Picker",
        "device_placeholder": "Please type device name(% for all)",
        "schedule_picker": "Schedules Picker",
        "schedules": "Schedules",
        "schedules_placeholder": "Type or choose a schedules",
        "title": "Leosac Builtin Access Point"
    },
    "link_to_profil": "Link to the user profile",
    "loading": {
        "message": "The page is being loaded in the background. It shouldn't take too long. <br> If the page doesn't load in a few seconds, try refreshing it.",
        "title": "Please wait"
    },
    "logicalRoot": "Logical zone",
    "login": "Login",
    "login-form": {
        "submit": "Sign in",
        "username_password_required": "Username and password are required.",
        "welcome": "Please sign in"
    },
    "logout": "Logout",
    "manager": "Manager",
    "mapping": "Mapping",
    "mask": "Mask",
    "member": "Member",
    "members": "Members",
    "message": "Message",
    "minutes_ago": "{{minutes}} minutes ago",
    "module": "Module",
    "modules": "Modules",
    "modules_both": "Loaded modules",
    "modules_both_desc": "Which modules are running through the server and the client.",
    "modules_client": "Client modules",
    "modules_client_desc": "Which modules are running through the client.",
    "modules_client_error": "These modules should have a server part to properly work: ",
    "modules_desc": "Modules that are launched.",
    "modules_server": "Server modules",
    "modules_server_desc": "Which modules are running through the server.",
    "modules_server_error": "These modules should have a client part to properly work: ",
    "modules_wizard": "Wizards",
    "modules_wizard_desc": "Which modules does the wizards need.",
    "name": "Name",
    "name_app": "Application name",
    "no": "No",
    "not_available": "Not Available",
    "notes": "Notes",
    "notes_placeholder": "A few optional notes...",
    "on": "on",
    "operator": "Operator",
    "other": "Other",
    "overview": {
        "intro": "This page provide an overview of some of the configuration value of the {{appname}} system.",
        "last_logs": "Last logs",
        "logs_per_page": "Log per page",
        "total_logs": "There is a total of {{totalLogs}} log messages."
    },
    "owner": "Owner",
    "owner_": "Owner:",
    "page_size": "Page Size",
    "parameter": "Parameter",
    "parameters": "Parameters",
    "password": "Password",
    "password-change": {
        "current_pw": "Current password",
        "new_pw": "New password",
        "submit": "Change password",
        "successfully_changed": "Password successfully changed."
    },
    "pending": "Pending",
    "pending_updates": "Pending updates",
    "physicalRoot": "Physical zone",
    "pincode": "Pin Code",
    "profile": {
        "fail_update": "Error while updating profile",
        "profile_updated": "Profile has been updated",
        "title": "User Profile"
    },
    "rank": "Rank",
    "rank_group": "Rank in group",
    "reader": "Reader",
    "refresh": "Refresh",
    "remove": "Remove",
    "remove_selected": "Remove selected",
    "remove_timeframe": "Remove timeframe",
    "restart": "Restart",
    "restart_info": "This button allow to restart the server",
    "save": "Save",
    "schedule": {
        "add_door": "Add door",
        "add_mapping": "Add mapping",
        "asfield": "Schedule:",
        "create": {
            "title": "Create schedule"
        },
        "create_note": "Please fill the form below in order to create a new schedule.",
        "create_schedule": "Create Schedule",
        "credential_schedule": "This shows schedules and their mapping that apply <i>directly</i> to the credential.",
        "delete_confirm": "Are you sure you wish to delete this schedule?",
        "doors": "Doors",
        "end_time": "End Time",
        "friday": "Friday",
        "grant_to_doors": "Grants access to the following doors:",
        "group_schedule": "This shows schedules and their mapping that apply <i>directly</i> to the group.",
        "join_mapping": "Join mapping",
        "list": {
            "title": "Schedule list"
        },
        "mapping": "Mapping: {{alias}}",
        "mapping_list": "Below are all the mapping linked to the schedule.",
        "monday": "Monday",
        "no_doors": "This mapping maps to no doors. This means it will have no effect.",
        "no_mapping": "There is no mapping",
        "no_timeframe": "This schedule has no timeframes. This means it will have no effect.",
        "placeholder": "MySchedule",
        "remove_from_mapping": "Remove from mapping",
        "remove_from_mapping_confirm": "Are you sure you want to leave this mapping?",
        "saturday": "Saturday",
        "start_time": "Start Time",
        "sunday": "Sunday",
        "thursday": "Thursday",
        "title": "Schedule",
        "tuesday": "Tuesday",
        "type": "Enter a schedule's name (or %)",
        "type_door": "Start typing a door's alias",
        "user_schedule": "This shows schedules and their mapping that apply <i>directly</i> to you. It wont show schedule that are mapped through your groups or credentials.",
        "wednesday": "Wednesday"
    },
    "schedules": "Schedules",
    "search": "Search",
    "security": "Security",
    "select_all": "Select all",
    "settings": "Settings",
    "start_date": "Start date",
    "submit": "Submit",
    "summary": "Summary",
    "supervisor": "Supervisor",
    "switch": "Switch languages",
    "switch_en": "Switch to english",
    "switch_fr": "Passer en français",
    "system-overview": {
        "title": "System Overview"
    },
    "table": {
        "group_id": "Group #Id",
        "group_name": "Group Name",
        "join_date": "Join date",
        "total_member": "Total members"
    },
    "test": "test",
    "timeframe": "Time Frame",
    "timeframes": "Time Frames",
    "timeframes_": "Time Frames:",
    "timestamp": "Timestamp",
    "to": "to",
    "toggle_all": "Toggle all",
    "total_page": "Total Page",
    "tree-view": {
        "title": "Zone Overview"
    },
    "type": "Type",
    "typeDoor": "Door",
    "typeGroup": "Group",
    "typePinCode": "Pin code",
    "typeRfidCard": "RFID card",
    "typeSchedule": "Schedule",
    "typeUser": "User",
    "typeZone": "Zone",
    "type_zone": "Zone type (can be logical or physical)",
    "unknown": "Unknown",
    "update": {
        "check_update": "Check Updates",
        "everything_up_to_date": "Everything is up to date",
        "filter_access_point": "Filter by access point",
        "pending_check_update": "Checking for updates. Please wait.",
        "potential_update": "Potential updates",
        "status": "status",
        "table": {
            "checkpoint": "Checkpoint",
            "generated_at": "Generation date",
            "target_alias": "Target Access Point Alias",
            "update_needed": "Need update ?",
            "updated_at": "Status change date"
        },
        "title": "Update Management",
        "update_acked": "Update has been acknowledged.",
        "update_acked_failed": "Failed to acknowledged update.",
        "update_cancel_failed": "Cancelling the update has failed.",
        "update_cancelled": "Update bas been cancelled."
    },
    "update_history": "Update history",
    "uptime": "Up time",
    "uptime_descr": "How long has {{appname}} been running. (HH:MM:SS)",
    "url": "Url",
    "user": "User",
    "username": "Username",
    "username_": "Username:",
    "users": {
        "add_credential": "Add credential",
        "add_group": "Add group",
        "add_to_group": "Add to group",
        "add_user": "Add user",
        "create": {
            "account_info": "Account Information",
            "fail_create": "Failed to create new user.",
            "title": "Create New User",
            "user_create": "User successfully created."
        },
        "create_group": "Create Group",
        "create_new_group": "Create New Group",
        "create_new_user": "Create new user",
        "delete_group_confirm": "Are you sure you wish to delete this group?",
        "error": {
            "add_error": "Failed to add user to group",
            "add_success": "Successfully added user to group",
            "find_error": "Cannot find this user."
        },
        "group_info": "Group Information",
        "group_name": "Group Name",
        "list": {
            "title": "User list"
        },
        "list_groups_notice": "Note that the \"Total members\" entry may not reflect the real number of members. This is a permission-related issue.",
        "members": "Total members",
        "remove_from_group": "Are you sure you wish to remove {{username}} from the group?",
        "title": "Users",
        "type_group": "Start typing a group's name",
        "type_user": "Start typing a user's name"
    },
    "validity": "Validity",
    "value": "Value",
    "viewer": "Viewer",
    "w": {
        "card_csn": "Card Serial Number",
        "credentials": "Credentials",
        "door": "Door",
        "overview": "Overview",
        "room_number": "Room Number",
        "severity": "Severity",
        "state": "State",
        "update": "Update",
        "user_id": "User Id"
    },
    "wait": "Please wait, fetching and preparing data.",
    "yes": "Yes",
    "zone": {
        "add_children": "Add a child zone",
        "child": "Children",
        "create": {
            "title": "Create zone"
        },
        "create_zone": "Add Zone",
        "delete_confirm": "Are you sure you wish to delete this zone ?",
        "error": {
            "edited_error": "An error occurred while editing the door",
            "edited_success": "Zone successfully edited",
            "error_added": "An error occurred while adding the door",
            "error_removed": "An error occurred while removing the door",
            "successfully_added": "Door successfully added",
            "successfully_removed": "Door successfully removed"
        },
        "list": {
            "title": "Zone list"
        },
        "placeholder": "MySuperZone",
        "remove_from_zone": "Remove from zone",
        "title": "Zone",
        "tree_view": "Zones overview",
        "type": {
            "format": "0 is for Logical, 1 is for physical",
            "logical": "Logical",
            "physical": "Physical"
        },
        "type_zone": "Start typing a zone's alias"
    },
    "zones": "Zones"
};
