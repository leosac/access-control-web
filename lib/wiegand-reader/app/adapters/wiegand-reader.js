...
export default class WiegandReaderAdapter extends Adapter {
    @service('websocket')
    ws;
    ...
    findRecord(store, type, id) {
        ...
        {
            this.ws.sendJson('wiegand-reader.read', {reader_id: id}).then(
                (data) => resolve(data),
    ...
    findAll() {
        return new Promise((resolve, reject) =>
        {
            this.ws.sendJson('wiegand-reader.read', {reader_id: 0}).then(
                (data) => resolve(data),
                (failure) => reject(failure)
            );
        });
    ...
    createRecord(store, type, snapshot) {
        ...
        return new Promise((resolve, reject) =>
        {
            this.ws.sendJson('wiegand-reader.create', {
    ...
    updateRecord(store, type, snapshot) {
        ...
        return new Promise((resolve, reject) =>
        {
            this.ws.sendJson('wiegand-reader.update', params).then((data) => resolve(data),
                (failure) => reject(failure));
        });
    }
    ...
    deleteRecord(store, type, snapshot) {
        ...
        return new Promise((resolve, reject) =>
        {
            this.ws.sendJson('wiegand-reader.delete', {reader_id: reader_id}).then(
                (data) => resolve(data),
                (failure) => reject(failure));
        });
    }
