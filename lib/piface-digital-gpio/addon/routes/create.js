    ...
    actions: {
        createPifaceDigitalGpio()
        {
            this.modelFor('create').save().then((d) =>
                {
                    this.get('flashMessages').success(this.get('i18n').t('piface-digital-gpio.error.create_success'));
                    this.transitionTo('piface-digital-gpio', d.get('id'));
                },
                () =>
                {
                    this.get('flashMessages').danger(this.get('i18n').t('piface-digital-gpio.error.create_error'));
                });
        }
    }
});
